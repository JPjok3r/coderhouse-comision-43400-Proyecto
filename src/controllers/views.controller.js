...
class ViewController{
    ...
    async productById(req, res) {
        ...
        try {
            ...
            res.status(200).render('productDetails', {prod, titlePage: "Detalle de Producto", user:req.session.user});
        } catch (error) {
            res.status(500).json({error});
        }
    }
    ...
    async carritoView(req, res) {
        ...
        try {
            ...
            res.status(200).render('cartDetail', {doc, titlePage: "Carrito", user:req.session.user});
        } catch (error) {
            res.status(500).json({error});
        }
    }
    ...
    adminViews(req, res){
        res.render("adminViews",{ layout: "adminMain", titlePage: "Administración del sitio", user:req.session.user});
    }
}
